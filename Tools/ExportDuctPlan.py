...
[실행 명령어]
기본 실행:
> python ExportDuctPlan.py
또는 Tools 디렉토리 기준:
> python ./tools/ExportDuctPlan.py

특정 파이프라인의 일환으로 또는 단독으로 터미널/PowerShell에서 실행하여, 
PostgreSQL DB에 저장된 덕트(Duct)의 3D 꼭짓점 정보 및 연결점(PoC) 정보를 바탕으로 
...
2. 덕트(Duct) 원본 데이터 조회
   - TB_DUCT 테이블로부터 덕트 인스턴스명, 유틸리티, 3D OBB(Oriented Bounding Box)의 8개 꼭짓점 X,Y,Z 좌표, 그리고 PoC 정보 목록(ID 배열, 좌표 배열, 구경 크기 배열)을 조회합니다.
3. 데이터 가공 및 기하 연산
   - parse_size_to_radius(): 규격화되지 않은 덕트/배관 크기 문자열(인치 형식 및 '600x400' 같은 사각 덕트 규격 포함)을 파싱하여 실제 mm 단위의 반지름(Radius) 실수값으로 환산합니다.
   - get_bottom_footprint(): 덕트의 3D OBB 정점 8개 중 Z값이 가장 낮은 4개 정점을 찾아 물리적 '바닥면'으로 결정하고, XY 평면 중심을 기준으로 시계/반시계 정렬하여 닫힌 외곽 다각형(2D Footprint)을 만듭니다.
4. 다중 포맷 파일 저장 및 시각화 (출력 경로: TopKGen/data/output)
   - export_dxf(): ezdxf 라이브러리로 새 R2010 도면을 생성하고, 덕트 외곽선 및 PoC 원(3D 고도 반영)을 레이어별로 자동 분할(유틸리티별 고유 색상 매핑)하여 저장합니다.
   - export_png(): Matplotlib을 이용해 덕트 면적 다각형과 PoC 원을 2D상에 축척에 맞춰 플로팅하고 유틸리티 범례와 함께 고해상도 PNG 이미지로 출력합니다.
   - export_shp(): shapefile 라이브러리를 사용해 GIS 호환이 가능한 3D 입체 덕트 다각형(POLYGONZ)과 PoC 점(POINTZ) 공간정보 파일(.shp, .dbf, .shx)을 각각 내보냅니다.
"""
...
def parse_size_to_radius(size_str: str) -> float:
    """
    덕트/배관 구경 규격 문자열(인치, 분수, mm 또는 사각 덕트 'Width x Height' 형식)을 분석하여 
    실제 배관/덕트 연결구의 mm 단위 반지름(Radius) 실수값으로 환산하는 함수입니다.
    
    [인자 (Arguments)]
    - size_str (str): 크기를 나타내는 원본 규격 문자열 (예: '2B', '600x400', '1 1/2', '250', '3/4' 등)
    
    [반환값 (Returns)]
    - float: mm 단위로 계산된 덕트/배관의 대표 반지름 값 (파싱 에러 시 기본값 25.0 mm를 반환)
    
    [주요 변수 및 동작 개요]
    - w, h: 사각 덕트 규격('600x400' 또는 '600*400') 파싱 시 폭과 높이 값을 추출
    - whole, frac: 대분수(예: '1 1/2') 파싱 시 정수부와 분수부를 임시 분할 저장
    - num, den: 분수(예: '3/4') 파싱 시 분자와 분모 분할 저장
    - val: 파싱 완료된 구경 지름 실수값
    - 대표 반경 환산 규칙:
      * 사각 덕트는 (폭 + 높이) / 4 를 대표 반지름으로 간주
    ...
    try:
        # 사각 덕트 형태인 경우 (예: '600X400' 또는 '600*400')
        if 'X' in size_str:
...
def get_bottom_footprint(obb_3d):
    """
    덕트의 3D 공간 상 8개 정점 좌표(OBB)를 분석하여 물리적인 바닥면(Bottom Face)을 검출하고, 
    XY 평면상에서 닫힌 정밀 다각형을 그리기 쉽도록 중심점 대비 방향성 각도 기준 정렬을 수행합니다.
    
    [인자 (Arguments)]
    - obb_3d (dict): 덕트의 OBB 3D 8개 모퉁이 점 좌표 정보 딕셔너리
...
def fetch_data(conn):
    ...
    with conn.cursor() as cur:
        ...
        for row in cur.fetchall():
            ...
            # 덕트의 3D 바닥면 다각형 정점 계산 및 2D XY 평면 투영(폴리곤 정보 추출)
...
def export_dxf(ducts, out_path):
    """
    덕트 바닥 외곽선과 PoC들을 도면 요소로 변환하여 AutoCAD용 DXF CAD 파일로 내보냅니다.
    
    [인자 (Arguments)]
    - ducts (list of dict): fetch_data()에서 정밀 파싱 완료되어 넘어온 덕트 데이터 목록
    - out_path (str): 출력되어 저장될 최종 DXF 파일의 절대 경로
    
    [주요 기하 드로잉 동작]
    - ezdxf.new('R2010'): AutoCAD 호환을 위해 R2010 DXF 규격의 새 문서를 생성합니다.
    - msp.add_lwpolyline(): 정렬 완료된 2D 바닥 외곽선을 'DUCT' 레이어에 흰색 닫힌(close=True) 선으로 그립니다.
    ...
    # 1. 덕트 레이어 및 유틸리티 종류별 CAD 전용 레이어 생성
    doc.layers.add('DUCT', color=ezdxf.colors.WHITE)
    for u in CAD_COLORS.keys():
        doc.layers.add(f'POC_{u}', color=CAD_COLORS[u])

    for duct in ducts:
        # 덕트 바닥면 영역 2D 다각형 폴리라인 추가
        msp.add_lwpolyline(duct['poly'], close=True, dxfattribs={'layer': 'DUCT'})
...
def export_png(ducts, out_path):
    """
    Matplotlib 플로팅 라이브러리를 이용하여 전체 덕트 영역과 PoC(연결구)의 레이아웃을 
    유틸리티 범례와 함께 2D 평면 도면 이미지(PNG)로 가시화하여 저장합니다.
    
    [인자 (Arguments)]
    - ducts (list of dict): 전체 덕트 및 PoC의 기하 정보 목록
    - out_path (str): 저장할 시각화 PNG 이미지 파일의 절대 경로
    
    [주요 변수 및 흐름]
    - fig, ax: 15인치 X 15인치 크기의 고해상도(dpi=300) 도면 판넬을 구성
    - Polygon: 옅은 황갈색 채움(#FFE4C4) 및 검은 외곽선을 가지는 덕트 2D 풋프린트 다각형 패치 객체
    ...
    for duct in ducts:
        # 덕트 바닥 다각형 Matplotlib 패치 인스턴스 생성 및 배치 (#FFE4C4: 비스크 황갈색으로 대비 적용)
    ...
    plt.title('Duct Floor Plan & PoCs')
    plt.xlabel('X (mm)')
    plt.ylabel('Y (mm)')
...
def export_shp(ducts, out_dir):
    """
    GIS 공간정보 시스템 및 외부 3D 프로그램과의 원활한 벡터 연동을 위하여 
    pyshp 라이브러리를 사용해 덕트 영역(POLYGONZ) 및 PoC(POINTZ) Shapefile을 빌드합니다.
    
    [인자 (Arguments)]
    - ducts (list of dict): 기하 데이터 정보 목록
    - out_dir (str): GIS 쉐이프 파일 세트가 내보내질 최상위 디렉토리
    
    [공간 정보 쓰기 로직 흐름]
    1. 덕트 Shapefile 쓰기 (POLYGONZ - Z 좌표를 가지는 3D 다각형 타입):
       - 'NAME' 문자열 속성 필드를 정의합니다.
       - 정렬된 3D 바닥 정점 4개를 바탕으로 첫 정점과 끝 정점을 닫아서 루프(part)를 만들고 기록합니다.
       - 덕트명(INSTANCE_NAME)을 Attribute Record에 연동합니다.
    2. PoC Shapefile 쓰기 (POINTZ - Z 좌표를 가지는 3D 점 타입):
       - 'DUCT_NAME'(덕트명), 'UTILITY'(유틸리티 종류), 'RADIUS'(PoC 반지름) 3개의 속성 필드를 구성합니다.
       - PoC의 X, Y, Z 절대 좌표를 점(PointZ) 요소로 추가하고, 각 PoC의 속성을 레코드 테이블에 대입합니다.
    """
    duct_shp_path = os.path.join(out_dir, "ducts")
    poc_shp_path = os.path.join(out_dir, "duct_pocs")
    
    # 1. 덕트 3D 바닥 다각형 정보 내보내기 (POLYGONZ)
    with shapefile.Writer(duct_shp_path, shapeType=shapefile.POLYGONZ) as w:
        w.field("NAME", "C", "50")
        for duct in ducts:
            bf = duct['bottom_face_3d']
    ...
    with shapefile.Writer(poc_shp_path, shapeType=shapefile.POINTZ) as w:
        ...
        for duct in ducts:
            for poc in duct['pocs']:
                ...
                w.record(duct['name'], poc['utility'], poc['radius'])
                
    print(f"Duct PoC Shapefile saved to {poc_shp_path}.shp")
...
def main():
    ...
    - fetch_data() 실행: 원본 덕트 기하와 PoC 관계 맵 일괄 가공 적재
    - 출력 저장 대상 경로(TopKGen/data/output) 폴더 자동 생성 검증
    - export_dxf(), export_png(), export_shp() 순차 실행 및 저장 완료
    """
    parser = argparse.ArgumentParser(description="Export duct plan drawings and GIS files.")
    ...
    dxf_path = os.path.join(out_dir, "ducts_plan.dxf")
    png_path = os.path.join(out_dir, "ducts_plan.png")
    
    # 3대 주요 포맷(DXF, PNG, GIS Shapefile) 일괄 출력 구동
    export_dxf(ducts, dxf_path)
    export_png(ducts, png_path)
    export_shp(ducts, out_dir)
