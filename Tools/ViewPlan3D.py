...
def fetch_category_data(cur, table_name, utility_map, level_filter, limit=None, use_aabb=True):
    """
    지정된 테이블로부터 OBB/AABB 및 PoC 정보를 데이터베이스로부터 쿼리하여 리스트로 정리합니다.
    """
    if use_aabb:
        query = f'''
    ...
    for row in cur.fetchall():
        if use_aabb:
            ...
            name, utility_col, level = row[0:3]
            raw_v = row[3:27]
            poc_ids, poc_pos, poc_sizes = row[27:30]
            vertices = [tuple(raw_v[i:i+3]) for i in range(0, 24, 3)]
...
def main():
    ...
    eq_rows = []
    for row in cur.fetchall():
        min_x, min_y, min_z, max_x, max_y, max_z = row[3:9]
        vertices = [
            (min_x, min_y, min_z),
            (max_x, min_y, min_z),
            (max_x, max_y, min_z),
        ...
        vertices = sort_box_vertices(vertices)
        pocs = []
        poc_ids, poc_pos, poc_sizes = row[9:12]
        if poc_pos:
            try:
                pos_list = json.loads(poc_pos)
                id_list = json.loads(poc_ids) if poc_ids else []
    ...
    ducts = fetch_category_data(cur, "TB_DUCT", utility_map, level_filter, use_aabb=True)
    
    print("Fetching Laterals...")
    laterals = fetch_category_data(cur, "TB_LATERAL_PIPE", utility_map, level_filter, limit=args.limit, use_aabb=True)
