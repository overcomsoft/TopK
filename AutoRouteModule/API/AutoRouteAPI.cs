...
namespace AutoRouteModule.API
{
    ...
    public static class AutoRouteAPI
    {
        ...
        {
            ValidatePathArguments(start, goal, startDirection, goalDirection, diameter);
        ...
        /// <summary>
        /// 일관된 인자 순서로 시작 지점에서 목표 지점까지의 경로를 비동기로 탐색합니다.
        /// </summary>
        public static void FindPath(
            Vector3 start,
            Vector3 goal,
        ...
        {
            ValidatePathArguments(start, goal, startDirection, goalDirection, diameter);
            if (options.HasValue) ValidateOptions(options.Value);
        ...
        {
            ValidateParallelArguments(pipes, groupCenterGoal, startDirection, goalDirection);
            if (onFinish == null) throw new ArgumentNullException(nameof(onFinish));
        ...
        {
            ValidateObstacles(obstacleObbs);
            if (onFinish == null) throw new ArgumentNullException(nameof(onFinish));
        ...
        {
            ValidateObstacle(obstacle, nameof(obstacle));
            if (onFinish == null) throw new ArgumentNullException(nameof(onFinish));
        ...
        {
            if (!IsFinite(octreeMinLeafSize) || octreeMinLeafSize <= 0)
                throw new ArgumentOutOfRangeException(nameof(octreeMinLeafSize));
            if (octreeMaxDepth < 0)
                throw new ArgumentOutOfRangeException(nameof(octreeMaxDepth));
            if (bvhThreshold < 1)
                throw new ArgumentOutOfRangeException(nameof(bvhThreshold));
            ...
            float diameter)
        {
            ...
            ValidateVector(goal, nameof(goal));
            ValidateDirection(startDirection, nameof(startDirection));
            ValidateDirection(goalDirection, nameof(goalDirection));
            if (!IsFinite(diameter) || diameter <= 0)
                throw new ArgumentOutOfRangeException(nameof(diameter), "Diameter must be finite and greater than zero.");
        }

        private static void ValidateParallelArguments(
            List<GroupPipeSpec> pipes,
            Vector3 groupCenterGoal,
            DirectionType startDirection,
            DirectionType goalDirection)
        {
            ...
            if (pipes.Count == 0) throw new ArgumentException("At least one pipe is required.", nameof(pipes));
            ValidateVector(groupCenterGoal, nameof(groupCenterGoal));
            ValidateDirection(startDirection, nameof(startDirection));
            ValidateDirection(goalDirection, nameof(goalDirection));
            for (int i = 0; i < pipes.Count; i++)
            {
                ValidateVector(pipes[i].Start, $"{nameof(pipes)}[{i}].Start");
                if (!IsFinite(pipes[i].Diameter) || pipes[i].Diameter <= 0)
                    throw new ArgumentOutOfRangeException(nameof(pipes), $"Pipe diameter at index {i} must be finite and greater than zero.");
            }
        }

        private static void ValidateWaypoints(List<Vector3> waypoints)
        {
            if (waypoints == null) throw new ArgumentNullException(nameof(waypoints));
            for (int i = 0; i < waypoints.Count; i++)
                ValidateVector(waypoints[i], $"{nameof(waypoints)}[{i}]");
        }

        private static void ValidateOptions(PathFindOptions options)
        {
            if (!IsFinite(options.HeuristicWeight) || options.HeuristicWeight < 1f)
                throw new ArgumentOutOfRangeException(nameof(options), "HeuristicWeight must be finite and at least 1.0.");
            if (!IsFinite(options.MinStraightDistance) || options.MinStraightDistance < 0)
                throw new ArgumentOutOfRangeException(nameof(options), "MinStraightDistance must be finite and non-negative.");
            if (options.MaxSearchNodes < 0 || options.TimeoutMilliseconds < 0)
                throw new ArgumentOutOfRangeException(nameof(options), "Search limits must be non-negative.");
        }

        private static void ValidateObstacles(List<OBB> obstacles)
        {
            if (obstacles == null) throw new ArgumentNullException(nameof(obstacles));
            for (int i = 0; i < obstacles.Count; i++)
                ValidateObstacle(obstacles[i], $"{nameof(obstacles)}[{i}]");
        }

        private static void ValidateObstacle(OBB obstacle, string name)
        {
            ValidateVector(obstacle.Center, $"{name}.Center");
            ValidateVector(obstacle.Extents, $"{name}.Extents");
            if (obstacle.Extents.X < 0 || obstacle.Extents.Y < 0 || obstacle.Extents.Z < 0)
                throw new ArgumentOutOfRangeException(name, "OBB extents must be non-negative.");
            if (obstacle.Axes == null || obstacle.Axes.Length != 3)
                throw new ArgumentException("OBB must contain exactly three axes.", name);
            for (int i = 0; i < obstacle.Axes.Length; i++)
            {
                ValidateVector(obstacle.Axes[i], $"{name}.Axes[{i}]");
                float length = obstacle.Axes[i].Length();
                if (length < 0.999f || length > 1.001f)
                    throw new ArgumentException("OBB axes must be normalized.", name);
            }
            if (Math.Abs(Vector3.Dot(obstacle.Axes[0], obstacle.Axes[1])) > 0.001f ||
                Math.Abs(Vector3.Dot(obstacle.Axes[0], obstacle.Axes[2])) > 0.001f ||
                Math.Abs(Vector3.Dot(obstacle.Axes[1], obstacle.Axes[2])) > 0.001f)
            {
                throw new ArgumentException("OBB axes must be mutually orthogonal.", name);
            }
        }
        ...
        private static void ValidateDirection(DirectionType direction, string name)
        {
